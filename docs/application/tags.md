...
**Application tags** limit who can sign in: only users with at least one tag that matches the application’s tags can access it. Admins and global admins are not restricted. Users and applications in the `built-in` organization are also not restricted.
