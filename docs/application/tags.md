...
**Application tags** limit who can sign in: only users with at least one tag that matches the application’s tags can access it. Admins and global admins are not restricted. In addition, users and applications that are in the `built-in` organization are also not restricted.
