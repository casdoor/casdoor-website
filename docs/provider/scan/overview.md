...
**Scan providers** (Category: **Scan**) scan a set of network address ranges to discover reachable servers. The discovered servers can then be managed in Casdoor — for example, as [MCP](/docs/how-to-connect/mcp/overview) servers on the **Servers** page.
...
Probes one or more intranet CIDR ranges over HTTP and reports which hosts are online and which of them expose an MCP server. For every online host, Casdoor tries each configured port/path combination and records the endpoints that respond as MCP servers.
