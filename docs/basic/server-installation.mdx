...
- `copyrequestbody` determines whether to copy the request body for later use, enabled by default.
...
- `initDataFile` specifies the path to the data initialization file. If not set, Casdoor looks for `init_data.json` in the root directory. See [Data Initialization](/docs/deployment/data-initialization) for details.
