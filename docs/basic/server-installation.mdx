...
For **Chinese** users, in order to download the GO dependency successfully, you need to change the configuration of GOPROXY. We strongly recommend that you update your [proxy server](https://goproxy.cn/).
...
Despite all the configurable fields, as a beginner, you only need to modify two items: `driverName` and `dataSourceName` based on your database. This database will be used by Casdoor to store all data, including users, organizations, applications and so on.
  ...
  - `defaultStorageProvider` is the default file storage service name. If you need to use file storage services such as avatar upload, you need to set up a storage provider and apply it in your application. See [**storage**](https://casdoor.org/docs/provider/storage/storage) for details.
