...
After downloading, extract the archive and run the `casdoor` executable. The binaries include the web frontend and a sample configuration file (`conf/app.conf`). You'll need to configure the database connection before running (see [Configuration](#configuration)).
