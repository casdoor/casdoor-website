...
- Set Proxy:
  ...
  In Docker Compose, the build command is supported to build the images needed to run the service. However, We have google-related services and some builds need to download foreign resources (such as golang builds), while the download fails due to network reasons,  so we can only resort to the proxy.

  When building, you can specify parameters, or set environment variables.

  The first method, by specifying build parameters:
  ...
  The second method, by setting the environment variable:
